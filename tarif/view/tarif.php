				...
				<div class="card">
					<div class="card-content wizard-content">
						...
						<form action="#" id="demoForm" class="myForm validation-wizard wizard-circle m-t-40 ">
							...
							<section id="section_one">
								...
								<br>
								<div class="row dr">
									<span style="margin-left: 2%;">Statut</span> <br>
									<p class='col s3'>
										<label>
								...
								<!-- <div class="card-action">
									<a href="#" id="href-next" class="right btn modal-action modal-close waves-effect waves-light cyan">Suivant</a>
								</div> -->
								<br>
					...
					<section id="section_two">
						<div style="background: #ededed; padding: 6px 0; border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;" class="row">
							...
							<div class="col s2">
								<span style="color:#000">Entité</span>
							</div>
							<div class="col s2">
...
<script>
	...
	function fetch_unite() {
		$.ajax({
			url: "<?php echo DOL_URL_ROOT; ?>/code4/unite",
			type: "post",
			dataType: "json",
			success: function(data) {
				...
				for (i in data) {
					...
					tbody += "<option value=" + data[i]['rowid'] + ">" + data[i]['short_label'] + "</option>";
				}
				$(".UnitForm").html(tbody);
			}
		});
	...
	function fetch_entity() {
		$.ajax({
			url: "<?php echo DOL_URL_ROOT; ?>/code4/entity",
			type: "post",
			dataType: "json",
			success: function(data) {
				// console.log(data);
				var tbody = "";
				for (i in data) {
					tbody += "<option value=" + data[i]['rowid'] + ">" + data[i]['label'] + "</option>";
				}
				$(".name_entity").html(tbody);
			}
		});
	...
	function fetch_Product(b) {
		var entity = "<?php echo $_SESSION['dol_entity'] ?>";
		$.ajax({
			url: "<?php echo DOL_URL_ROOT; ?>/code4/product",
			type: "post",
			dataType: "json",
			data: {
				entity: entity
			...
			success: function(data) {
				var tbody = "";
				tbody += "<option > Selectionner produit </option>";
		...
		$(add_button).click(function(e) {
			...
			if (x < max_fields) {
				...
				$(wrapper).append(
					...
					<div class="row">
							<div class=" col s2">
							<select class="required name_entity" >
							...
							<div class=" col s2">
								<input type="text" data-count="` + x + `" name="prix_ht" id="prix_ht` + x + `" class="prix_HT">
							</div>
							...
							<div class=" col s2">
								<input type="text" id="tttc` + x + `" class="TTC TTC` + x + `" disabled>
							</div>
		...
		`
			...
			setTimeout(function() {
				for (var i = 0; i < pr_select.length; i++) {
					$("#product_select" + x + " option[value='" + pr_select[i] + "']").remove();
				}
			}, 300);
		});
	...
	var pr_select = [];
	$(document).on("change", '.produit_select', function(e) {
		e.preventDefault();
		var data = $(this).data('selectp');

		$('.produit_select').each(function() {
			pr_select.push($(this).val());
		});
	...
	$(document).on("click", "#add-product", function(e) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('.name_entity').each(function() {
					name_entity.push($(this).val());
				});
				$('.produit_select').each(function() {
